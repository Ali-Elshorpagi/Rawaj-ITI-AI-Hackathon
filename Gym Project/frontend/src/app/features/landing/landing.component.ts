...
import { SubscriptionPlan, GymClass, ApiResponse, PaginatedResponse } from '../../models/interfaces';
...
export class LandingComponent implements OnInit {
  ...
  private loadPlans(): void {
    this.http.get<ApiResponse<SubscriptionPlan[]>>(`${environment.apiUrl}/plans`).subscribe({
      next: (res) => {
        this.plans.set(res.data.filter(p => p.isActive));
  ...
  private loadClasses(): void {
    this.http.get<PaginatedResponse<GymClass>>(`${environment.apiUrl}/classes?limit=6&status=scheduled`).subscribe({
      next: (res) => {
