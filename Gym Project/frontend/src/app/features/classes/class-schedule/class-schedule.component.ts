...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterLink, FormsModule, MatIconModule, MatButtonModule, MatDatepickerModule, MatNativeDateModule, MatFormFieldModule, MatInputModule, TranslateModule],
  template: `
    <div class="page">
      <div class="gd-page-header">
        ...
        <div style="display:flex;gap:10px;align-items:center">
          ...
          <a mat-stroked-button routerLink="/classes"><mat-icon>list</mat-icon> List</a>
        </div>
      </div>
              ...
              <div class="schedule-day__classes">
                @for (cls of day.classes; track cls.id) {
                  <div class="schedule-event" [style.background]="getEventColor(cls)">
                    ...
                    <div class="event-meta">{{ cls.participants.length }}/{{ cls.capacity }}</div>
                  </div>
                } @empty {
...
export class ClassScheduleComponent implements OnInit {
      ...
      next: (res) => {
        ...
        for (let i = 0; i < 7; i++) {
          ...
          columns.push({
            ...
            classes: classes.filter(c => new Date(c.startTime).toDateString() === dayStr),
          });
        }
  ...
  getEventColor(cls: GymClass): string {
    ...
    const idx = cls.title.charCodeAt(0) % colors.length;
    return colors[idx];
  }
}
