...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MatIconModule, MatButtonModule, MatFormFieldModule, MatInputModule, TranslateModule],
  template: `
    <div class="page">
      <div class="gd-page-header">
        ...
        <button mat-flat-button color="primary" (click)="openDialog()" *ngIf="auth.isAdmin()">
          <mat-icon>add</mat-icon> {{ 'trainers.addTrainer' | translate }}
        </button>
      </div>
          ...
          @for (trainer of trainers(); track trainer.id) {
            <div class="trainer-card gd-card">
              ...
              <div class="trainer-card__info">
                ...
                <p class="trainer-card__spec">{{ trainer.specialization.join(', ') }}</p>
                <span class="badge badge--{{ trainer.isActive ? 'active' : 'expired' }}">
                  {{ trainer.isActive ? 'Active' : 'Inactive' }}
                </span>
              </div>
              <div class="trainer-card__actions" *ngIf="auth.isAdmin()">
                <button mat-icon-button (click)="editTrainer(trainer)"><mat-icon>edit</mat-icon></button>
                <button mat-icon-button color="warn" (click)="deleteTrainer(trainer)"><mat-icon>delete</mat-icon></button>
              </div>
            </div>
          } @empty {
...
export class TrainerListComponent implements OnInit {
  ...
  openDialog(trainer?: Trainer): void {
    ...
    if (trainer) {
      this.form.patchValue({ ...trainer, specialization: trainer.specialization.join(', ') });
    } else {
      this.form.reset();
    }
