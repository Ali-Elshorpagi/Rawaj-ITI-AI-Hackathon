...
@Component({
            ...
            <div class="tab-content">
              <div class="gd-grid gd-grid--3">
                <div class="gd-card">
                  ...
                  <p class="info-item"><mat-icon>card_membership</mat-icon> {{ member()!.subscriptionPlan?.name ?? 'No plan' }}</p>
                  <p class="info-item"><mat-icon>event</mat-icon> Expires: {{ member()!.subscriptionEndDate ? (member()!.subscriptionEndDate | date) : 'N/A' }}</p>
                  <p class="info-item"><mat-icon>how_to_reg</mat-icon> Attendance: {{ member()!.totalAttendance }}</p>
                </div>
                <div class="gd-card">
                  <div class="gd-card__title">Contact</div>
                  <p class="info-item"><mat-icon>phone</mat-icon> {{ member()!.phone ?? 'N/A' }}</p>
                  <p class="info-item"><mat-icon>location_on</mat-icon> {{ member()!.address?.city ?? 'N/A' }}</p>
                </div>
                <div class="gd-card">
                  <div class="gd-card__title">Emergency Contact</div>
                  @if (member()!.emergencyContact) {
                    <p class="info-item"><mat-icon>person</mat-icon> {{ member()!.emergencyContact!.name }}</p>
                    <p class="info-item"><mat-icon>phone</mat-icon> {{ member()!.emergencyContact!.phone }}</p>
                    <p class="info-item"><mat-icon>group</mat-icon> {{ member()!.emergencyContact!.relationship }}</p>
                  } @else {
          ...
          @if (plansLoading()) {
              ...
              @for (plan of availablePlans(); track plan.id) {
                <div class="plan-option"
                  ...
                  <div class="plan-option__check">
                    <mat-icon>{{ selectedPlanId() === plan.id ? 'radio_button_checked' : 'radio_button_unchecked' }}</mat-icon>
                  </div>
            ...
            <div class="dialog-actions">
              ...
              <button mat-flat-button color="primary"
                ...
                <mat-icon>{{ checkoutLoading() ? 'sync' : 'open_in_new' }}</mat-icon>
  ...
  styles: [`
    ...
    .info-item { display: flex; align-items: center; gap: 8px; margin-bottom: 12px; mat-icon { font-size: 18px; width: 18px; height: 18px; color: var(--text-muted); } }
    ...
    .plan-option__check { color: var(--color-primary-500); mat-icon { font-size: 20px; width: 20px; height: 20px; } }
...
export class MemberDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.attendanceService.getMemberHistory(id).subscribe({
      next: (res) => { this.attendance.set(res.data); this.attendanceLoading.set(false); },
      error: () => this.attendanceLoading.set(false),
    });
    this.paymentsService.getMemberPayments(id).subscribe({
      next: (res) => { this.payments.set(res.data); this.paymentsLoading.set(false); },
  ...
  initiateCheckout(): void {
    ...
    this.paymentsService.createCheckout(memberId, planId).subscribe({
      next: (res) => {
