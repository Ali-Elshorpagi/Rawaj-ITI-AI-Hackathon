...
export class AuthService {
  ...
  // POST /api/auth/login  — role is required by the backend
  ...
  // OTP is auto-sent to email; call verifyOtp() next
  register(data: {
    name: string;
    email: string;
    password: string;
  ...
  // Alias kept for register component — delegates to register()
  ...
  // POST /api/auth/verify — verifies email with 4-digit OTP
  ...
  verifyRegisterOtp(payload: { email: string; otp: string }): Observable<ApiResponse<any>> {
    return this.verifyOtp(payload.email, payload.otp);
  }
