...
export class MemberController {
  ...
  async getByQR(req: Request, res: Response, next: NextFunction) {
    try {
      const result = await this.memberService.getMemberByQR(req.params.memberId);
      res.sendSuccess("Member fetched", result, 200);
    } catch (error) {
      next(error);
    }
  ...
  async updateStatus(req: Request, res: Response, next: NextFunction) {
    try {
      const result = await this.memberService.updateMemberStatus(
        req.params.id,
        req.body.status
      );
      res.sendSuccess("Status updated", result, 200);
    } catch (error) {
      next(error);
    }
  }
  ...
  async getStats(req: Request, res: Response, next: NextFunction) {
    try {
      const result = await this.memberService.getMemberStats();
      res.sendSuccess("Stats fetched", result, 200);
    } catch (error) {
      next(error);
    }
  }
