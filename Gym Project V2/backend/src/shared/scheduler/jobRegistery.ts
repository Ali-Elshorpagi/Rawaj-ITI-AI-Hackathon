...
import registerAutoCheckOutJob, { AUTO_CHECKOUT_JOB } from "./jobs/autoCheckOutJob";

export const initScheduler = async () => {
  registerCleanUpJob(agenda);
  registerAutoCheckOutJob(agenda);
  ...
  const autoCheckOutJobs = await agenda.jobs({ name: AUTO_CHECKOUT_JOB });
  if (autoCheckOutJobs.length === 0) {
    await scheduleRecurringJob(AUTO_CHECKOUT_JOB, "*/15 * * * *");
