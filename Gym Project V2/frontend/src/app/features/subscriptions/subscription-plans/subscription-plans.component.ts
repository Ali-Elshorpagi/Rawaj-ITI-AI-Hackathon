...
@Component({
    ...
    <div class="page">
      <div class="gd-page-header">
        ...
        <button mat-flat-button color="primary" (click)="openPlanDialog()" *ngIf="auth.isAdmin()">
          <i class="fa-solid fa-plus"></i> {{ 'subscriptions.addPlan' | translate }}
        </button>
      </div>
  ...
  styles: [`
    ...
    .plan-card {
      ...
      &__features { list-style: none; margin-bottom: 20px; li { display: flex; align-items: center; gap: 8px; font-size: 0.875rem; color: var(--text-secondary); margin-bottom: 8px; i { font-size: 16px; color: var(--color-success-600); } } }
...
export class SubscriptionPlansComponent implements OnInit {
  ...
  savePlan(): void {
    ...
    const obs = this.editingPlan()
      ? this.subscriptionsService.updatePlan(this.editingPlan()!.id ?? this.editingPlan()!._id!, data)
  ...
  async deletePlan(plan: SubscriptionPlan): Promise<void> {
    ...
    if (!confirmed) return;
    this.subscriptionsService.deletePlan(plan.id ?? plan._id!).subscribe({
