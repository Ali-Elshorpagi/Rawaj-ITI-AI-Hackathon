...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, MatButtonModule, MatSelectModule, MatFormFieldModule, MatPaginatorModule, TranslateModule],
  template: `
    <div class="page">
...
export class PaymentListComponent implements OnInit {
  ...
  loadPayments(): void {
    ...
    this.paymentsService.getAll(params).subscribe({
      next: (res) => { this.payments.set(res.data.items); this.meta.set(res.data.pagination); this.loading.set(false); },
  ...
  async refund(payment: Payment): Promise<void> {
    const confirmed = await this.modal.confirm({
      ...
      message: `Refund payment of $${payment.amount}? This will reverse the payment and cannot be undone.`,
      confirmText: 'Refund',
      cancelText: 'Cancel',
      type: 'warning',
    });
    if (!confirmed) return;
    this.paymentsService.refund(payment.id ?? payment._id!).subscribe({
