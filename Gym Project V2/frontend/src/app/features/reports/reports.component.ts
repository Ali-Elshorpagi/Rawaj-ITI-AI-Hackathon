...
@Component({
  selector: 'gd-reports',
  standalone: true,
  imports: [CommonModule, FormsModule, MatButtonModule, MatSelectModule, MatFormFieldModule, MatInputModule, MatDatepickerModule, MatNativeDateModule, TranslateModule, BaseChartDirective],
  template: `
    <div class="page">
          ...
          <div class="gd-grid gd-grid--4">
            @for (kpi of attendanceKPIs(); track $index) {
              <div class="gd-kpi-card">
                <div class="gd-kpi-card__icon" [style.background]="kpi.bg"><i [class]="kpi.icon" [style.color]="kpi.color"></i></div>
  ...
  styles: [`
    ...
    .tab-btn { display: flex; align-items: center; gap: 6px; padding: 8px 16px; border: none; border-radius: var(--radius-md); background: transparent; color: var(--text-secondary); font-size: 0.875rem; font-weight: 500; cursor: pointer; transition: all var(--transition-base); i { font-size: 18px; } }
...
export class ReportsComponent implements OnInit {
  ...
  readonly tabs = [
    { id: 'attendance', label: 'Attendance', icon: 'fa-solid fa-users' },
    { id: 'revenue', label: 'Revenue', icon: 'fa-solid fa-money-bill-wave' },
    { id: 'membership', label: 'Membership', icon: 'fa-solid fa-id-card' },
    { id: 'trainers', label: 'Trainer Performance', icon: 'fa-solid fa-person-running' },
  ];
  ...
  private processReportData(data: any): void {
    ...
    if (tab === 'attendance') {
      this.attendanceKPIs.set([
        { icon: 'fa-solid fa-users', label: 'Total Check-ins', value: data.total ?? 0, bg: '#EEF1F8', color: '#3F5587' },
        { icon: 'fa-solid fa-arrow-trend-up', label: 'Daily Average', value: data.dailyAverage?.toFixed(1) ?? '0', bg: '#F0FDF4', color: '#16A34A' },
        { icon: 'fa-solid fa-trophy', label: 'Peak Day', value: data.peakDay ?? '—', bg: '#FEF3C7', color: '#D97706' },
        { icon: 'fa-solid fa-dumbbell', label: 'Unique Members', value: data.uniqueMembers ?? 0, bg: '#FDF2F8', color: '#9333EA' },
      ]);
      ...
      this.revenueKPIs.set([
        { icon: 'fa-solid fa-money-bill-wave', label: 'Total Revenue', value: `$${Number(total).toFixed(0)}`, bg: '#EEF1F8', color: '#3F5587' },
        { icon: 'fa-solid fa-receipt', label: 'Total Transactions', value: data.totalTransactions ?? 0, bg: '#F0FDF4', color: '#16A34A' },
        { icon: 'fa-solid fa-arrow-trend-up', label: 'Avg Transaction', value: `$${Number(data.avgTransaction ?? 0).toFixed(0)}`, bg: '#FEF3C7', color: '#D97706' },
        { icon: 'fa-solid fa-rotate-left', label: 'Refunds', value: data.totalRefunds ?? 0, bg: '#FDF2F8', color: '#9333EA' },
      ]);
