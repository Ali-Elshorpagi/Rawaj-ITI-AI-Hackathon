...
@Component({
  selector: 'gd-profile',
  standalone: true,
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MatButtonModule, MatFormFieldModule, MatInputModule, MatSelectModule, MatDatepickerModule, MatNativeDateModule, MatTabsModule, TranslateModule],
  template: `
    <div class="page">
              ...
              <div class="tab-content gd-card">
                <form [formGroup]="profileForm" (ngSubmit)="saveProfile()">
                  ...
                  <mat-form-field appearance="outline">
                    ...
                    <span matSuffix [style.color]="auth.currentUser()?.isEmailVerified ? '#16a34a' : '#f59e0b'" style="display:flex;align-items:center;padding-right:4px">
                      <i [class]="auth.currentUser()?.isEmailVerified ? 'fa-solid fa-circle-check' : 'fa-solid fa-triangle-exclamation'"></i>
                    </span>
                  </mat-form-field>
                  <div class="form-row">
                    <mat-form-field appearance="outline">
  ...
  styles: [`
    ...
    .verified-badge { display: flex; align-items: center; gap: 4px; font-size: 0.8125rem; color: #16a34a; margin-top: 12px; i { font-size: 14px; } }
    ...
    .stat-icon { width: 44px; height: 44px; border-radius: 12px; display: flex; align-items: center; justify-content: center; margin: 0 auto 12px; i { font-size: 20px; } }
    .stat-value { font-size: 1.5rem; font-weight: 700; }
    .stat-label { font-size: 0.75rem; color: var(--text-muted); margin-top: 2px; }
    .last-login { display: flex; align-items: center; gap: 8px; font-size: 0.875rem; color: var(--text-muted); padding: 12px; background: var(--surface-bg); border-radius: var(--radius-md); i { font-size: 16px; } }
...
export class ProfileComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.activityStats.set([
      { icon: 'fa-solid fa-right-to-bracket', label: 'Total Logins', value: '—', bg: '#EEF1F8', color: '#3F5587' },
      { icon: 'fa-solid fa-calendar-day', label: 'Member Since', value: user?.createdAt ? new Date(user.createdAt).getFullYear() : '—', bg: '#F0FDF4', color: '#16A34A' },
      { icon: 'fa-solid fa-shield-halved', label: 'Role', value: user?.role ?? '—', bg: '#FEF3C7', color: '#D97706' },
    ]);
  }
