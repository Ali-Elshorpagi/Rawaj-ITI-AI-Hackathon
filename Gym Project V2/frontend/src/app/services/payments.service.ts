...
export class PaymentsService extends ApiService {
  createCheckout(planId: string): Observable<ApiResponse<{ sessionId: string; sessionUrl: string }>> {
    return this.post<ApiResponse<any>>('/payments/checkout', { planId });
  }

  verifySession(sessionId: string): Observable<ApiResponse<{ fulfilled: boolean; subscription?: any }>> {
    return this.post<ApiResponse<any>>('/payments/verify-session', { sessionId });
  }
